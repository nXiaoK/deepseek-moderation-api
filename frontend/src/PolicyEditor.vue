...
<template>
  ...
  <section v-if="tab === 'routing'" class="panel config-panel">
    ...
    <p class="hint">
      例如：两个主用通道设为优先级 1、权重 70 和 30；备用通道设为优先级 2。RPM
      可在“审核模型”中配置，实际分流还会考虑剩余额度，不保证每批请求比例一致。
    </p>
    <div class="form-grid">
      <label
        >总调用时限（毫秒）<input
    ...
    <p class="hint">
      ...
      只有一个可用审核通道时，免除失败冷却，但仍受并发和 RPM
      限制（缓存命中不调用模型）。限流遵守上游建议，达到失败阈值时取上游建议与失败冷却时间中的较长值。
