...
数据库迁移保存在 `internal/audit/migrations/`，应用通过 `audit_schema_migrations` 记录版本、文件名与校验值。首次接入迁移记录时会幂等应用当前基线，不清空既有业务数据；已执行的 SQL 文件不得直接修改，后续变更必须新增顺序迁移。程序会拒绝未知的更高版本、缺失历史或校验值不一致的数据库。
