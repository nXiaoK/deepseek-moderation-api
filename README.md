...
- 默认连续模型失败 3 次后冷却 30 分钟，在“审核策略 → 模型调度”中可设置连续失败次数（1～100）和冷却时间（1～1440 分钟）。旧策略未配置或 API 传入 0 时使用默认值。超时、无效输出、401/403 和其他上游错误均计入连续失败，成功调用清零；请求取消和总时限耗尽不触发冷却。冷却期间按原有优先级和同级权重使用其他通道，同级均不可用时切换到下一级，仍受最多调用次数和总时限约束。429 遵守有效 Retry-After（最多 24 小时），达到失败阈值时取其与配置冷却时间的较长值。冷却到期后只放行一个恢复试探，失败则重新冷却；没有定时付费探测。只有一个符合当前请求条件的通道时，忽略冷却、每次请求仍尝试调用（有效缓存仍优先返回），每次请求同一通道最多调用一次，并发限制仍生效。健康状态在单进程内按通道共享，冷却时长采用触发失败的策略设置；重启进程、保存通道或点击“清缓存 / 重试连接”可重置。
