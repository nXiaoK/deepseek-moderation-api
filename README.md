...
第一版支持文本及文本内容块数组，最多 64000 字符。正式审核接口收到超过 1 MiB 且不超过 32 MiB 的含图片请求时，自动跳过 `image_url` 图片内容块，仅审核文本；阈值按完整 JSON 请求体（含 Base64）计算。跳过后无文本或文本超过 64000 字符仍返回错误。审核记录明确标记“图片已跳过，仅审核文本”，响应带 `X-Audit-Input-Scope: text-only`；结果不覆盖图片内容。小于等于 1 MiB 的图片请求仍返回不支持图片的错误，纯文本请求和管理接口维持 1 MiB 上限。部署反向代理时也需允许至少 32 MiB 的请求体，否则请求会在到达本服务前被拒绝。模型调用不使用工具、不改变管理员的审核范围。试跑记录与正式流量隔离；未填写真实 DeepSeek 密钥前，不能进行真实模型审核。
