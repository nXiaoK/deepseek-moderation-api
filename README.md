...
图片 token 暂不支持调用前预估，带日/月预算限制的调用方只选择可估算费用的文本通道，不能使用文本估算绕过图片预算；普通调用可使用图片通道，按上游实际报告的用量和已有定价规则结算。第三方接口仍按原规则待核对。模型调用不使用工具、不改变管理员的审核范围。试跑记录与正式流量隔离；未填写真实模型密钥前，不能进行真实模型审核。
